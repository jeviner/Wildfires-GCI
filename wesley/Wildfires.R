...
##this step is for conecting to a database and pulling just one aspect from it, then disconnecting.
...
# Let's pick CA fires only.  We do this using the commaned in the dpyr package.
...
#How many fires occured within each year.  Is there an increasing trend?
...
CA_fires_by_year %>% ggplot(aes(x = FIRE_YEAR, y = n_fires/1000)) + 
  geom_bar(stat = 'identity', fill = 'orange') +
  geom_smooth(method = 'lm', se = FALSE, linetype = 'dashed', size = 0.4, color = 'red') + 
  labs(x = '', y = 'Number of wildfires (thousands)', title = 'CA Wildfires by Year')
...
#We can turn it nto size classes
